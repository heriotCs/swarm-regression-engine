...
# Suppress warnings that clutter the output during experiments
warnings.filterwarnings("ignore", category=RuntimeWarning)
warnings.filterwarnings("ignore", category=UserWarning)
warnings.filterwarnings("ignore", category=FutureWarning)
warnings.filterwarnings("ignore", category=ConvergenceWarning)

# Add project root so imports work when running from /experiments
...
# Ensures results folders exist before saving plots/metrics
def ensure_dirs():
    os.makedirs("results/plots", exist_ok=True)
    os.makedirs("results", exist_ok=True)


# Loads the UCI concrete dataset and performs preprocessing.
# Returns train/test split with standardisation.
def load_data(filepath: str = "Concrete_Data.xls"):
    ...
    # Normalise inputs for smoother optimisation
...
# Helper function to save a matplotlib figure to disk
...
def run_experiment(X_train, X_test, y_train, y_test):
    ...
    # Build a simple 1-hidden-layer regression network
    network = ANNBuilder.build_regression_network(input_size=8, hidden_layers=[10], seed=42)

    # Trainer object wraps the ANN and handles PSO optimisation
    ...
    # Manual PSO construction (for detailed control)
    ...
    # Run PSO on this ANN architecture
    ...
    # Load best weights into the network
...
# Compares several ANN architectures using PSO training
def run_multiple_experiments(X_train, X_test, y_train, y_test):
    ...
    for layers in architectures:
        ...
        # Faster runs with lower iteration count and silent mode
        trainer.pso = PSO(
            fitness=fitness,
            dim=trainer.ann_fitness.num_params,
            bounds=bounds,
    ...
    # Visual comparison of architecture performance
...
# Simple Adam/Backprop baseline for comparison
...
# Main script for running all experiments end-to-end
def main():
    ensure_dirs()
